...
# CRITICAL: app must be initialized FIRST before any routes
app = Flask(__name__)
...
def scan():
    ...
    try:
        ...
        # Timeout configured to 30 seconds to prevent indefinite hanging
        client = genai.Client(
            api_key=api_key,
        ...
        diagnostic_logs.append("GenAI Client initialized successfully.")
        
        prompt_text = f"Generate an optimized YouTube video title, a short SEO description, and 4 comma-separated tags tailored explicitly for a video file named: {filename}"
        ...
        for m_name in ['gemini-3.6-flash', 'gemini-3.5-flash']:
            try:
                ...
                diagnostic_logs.append(f"Model {m_name} skipped: {model_err}")
                continue
                
        if response_ai and hasattr(response_ai, 'text') and response_ai.text:
            ai_description = response_ai.text
        else:
            ai_description = "Error: Models took too long or are busy. Please try scanning again."
